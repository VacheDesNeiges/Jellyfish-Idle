...
struct ImGuiDataVarInfo;  // Variable information (e.g. to access style
...
// In a real application you should be using PRId64 and PRIu64 from
// <inttypes.h> (non-windows) and on Windows define them yourself.
#if defined(_MSC_VER) && !defined(__clang__)
...
#ifndef ImQsort
static inline void
ImQsort (void *base, size_t count, size_t size_of_element,
...
#endif
// - ImMin/ImMax/ImClamp/ImLerp/ImSwap are used by widgets which support
// variety of types: signed/unsigned int/long long float/double (Exceptionally
// using templates here but we could also redefine them for those types)
template <typename T>
static inline T
ImMin (T lhs, T rhs)
{
...
ImExponentialMovingAverage (float avg, float sample, int n)
{
  ...
  avg += sample / static_cast<float> (n);
  return avg;
}
IM_MSVC_RUNTIME_CHECKS_RESTORE
...
// Helper: ImVec2ih (2D vector, half-size integer, for long-term packed
// storage)
struct ImVec2ih
{
  short x, y;
...
struct IMGUI_API ImRect
{
  ...
  bool
  Contains (const ImVec2 &p) const
  {
    return p.x >= Min.x && p.y >= Min.y && p.x < Max.x && p.y < Max.y;
  }
  bool
  Contains (const ImRect &r) const
  {
    return r.Min.x >= Min.x && r.Min.y >= Min.y && r.Max.x <= Max.x
           && r.Max.y <= Max.y;
  }
  bool
  ContainsWithPad (const ImVec2 &p, const ImVec2 &pad) const
  {
    return p.x >= Min.x - pad.x && p.y >= Min.y - pad.y && p.x < Max.x + pad.x
           && p.y < Max.y + pad.y;
  }
  bool
  Overlaps (const ImRect &r) const
  {
...
#define IM_BITARRAY_CLEARBIT(_ARRAY, _N)                                      \
  ((_ARRAY[(_N) >> 5]                                                         \
    &= ~((ImU32)1 << ((                                                       \
             _N)&31)))) // Macro version of ImBitArrayClearBit(): ensure args
...
template <typename T> struct ImSpan
{
  ...
  // Utilities
  inline int
  index_from_ptr (const T *it) const
  {
...
template <int CHUNKS> struct ImSpanAllocator
{
  ...
  inline void
  Reserve (int n, size_t sz, int a = 4)
  {
  ...
  inline void
  SetArenaBasePtr (void *base_ptr)
  {
    BasePtr = (char *)base_ptr;
  }
  inline void *
  GetSpanPtrBegin (int n)
  {
...
// choosing segment that is perpendicular to X axis. Follow steps in the
// article from this starting condition and you will will get this result.
//
...
enum ImGuiSelectableFlagsPrivate_
{
  ...
  = 1 << 21, // (WIP) Auto-select when moved into. This is not exposed in
             // public API as to handle multi-select and modifiers we will need
             // user to explicitly control focus scope. May be replaced with a
             // BeginSelection() API.
...
struct IMGUI_API ImGuiInputTextState
{
  ...
  int
  GetUndoAvailCount () const
  {
    return Stb.undostate.undo_point;
  }
  int
  GetRedoAvailCount () const
  {
...
// that we need to perform the equivalent of NavApplyItemToResult() and which
// we can't infer in TreePop() Only stored when the node is a potential
// candidate for landing on a Left arrow jump.
...
// this 'ImGuiKeyOwner_None' but it was inconsistent with our pattern that
// _None values == 0 and quite dangerous. Also using _NoOwner makes the
...
enum ImGuiInputFlagsPrivate_
{
  ...
  ImGuiInputFlags_LockUntilRelease
  = 1
    << 21, // Further accesses to key data will require EXPLICIT owner ID
...
enum ImGuiNavMoveFlags_
{
  ...
  = 1 << 4, // Allow scoring and considering the current NavId as a move target
            ...
            // pressing PageDown actually needs to send a Up move request, if
            // we are pressing PageDown from the bottom-most item we need to
            // stay in place)
...
enum ImGuiDockNodeFlagsPrivate_
{
  ...
  = 1 << 18, //       // Any docked window will be automatically be focus-route
             //       chained (window->ParentWindowForFocusRoute set to this)
             //       so Shortcut() in this window can run when any docked
             //       window is focused.
...
struct IMGUI_API ImGuiDockNode
{
  ...
  void
  SetLocalFlags (ImGuiDockNodeFlags flags)
  {
    LocalFlags = flags;
    UpdateMergedFlags ();
  }
...
// We don't store style.Alpha: dock_node->LastBgColor embeds it and otherwise
// it would only affect the docking tab, which intuitively I would say we don't
// want to.
...
struct ImGuiViewportP : public ImGuiViewport
{
  ...
  // building rect)
  ImVec2
  CalcWorkRectPos (const ImVec2 &off_min) const
  {
    return ImVec2 (Pos.x + off_min.x, Pos.y + off_min.y);
  }
  ImVec2
  CalcWorkRectSize (const ImVec2 &off_min, const ImVec2 &off_max) const
  {
...
// user-facing api such as IsItemVisible() / IsRectVisible() which use
// "Visible" to mean "not clipped".
...
// This is designed to be stored in a single ImChunkStream (1 header followed
// by N ImGuiTableColumnSettings, etc.)
...
inline void
SetWindowParentWindowForFocusRoute (ImGuiWindow *window,
                                    ImGuiWindow *parent_window)
{
...
IMGUI_API void OpenPopupEx (ImGuiID id, ImGuiPopupFlags popup_flags
...
// which are much harder to design and implement than expected. I have a couple
// of private branches on this matter but it's not simple. For now implementing
// the easy ones.
...
// - Expected to be later promoted to public API, the prototypes are designed
// to replace existing ones (since owner_id can default to Any == 0)
...
//   matching current ones: SetKeyOwner() is called + route is granted for the
//   frame.
...
inline ImGuiDockNode *
DockNodeGetRootNode (ImGuiDockNode *node)
{
...
inline bool
DockNodeIsInHierarchyOf (ImGuiDockNode *node, ImGuiDockNode *parent)
{
  while (node)
    {
      if (node == parent)
...
inline int
DockNodeGetDepth (const ImGuiDockNode *node)
{
  int depth = 0;
  while (node->ParentNode)
    {
...
inline ImGuiID
DockNodeGetWindowMenuButtonId (const ImGuiDockNode *node)
{
...
// - DockBuilderSplitNode() create 2 child nodes within 1 node. The initial
// node becomes a parent node.
...
inline ImGuiDockNode *
DockBuilderGetCentralNode (ImGuiID node_id)
{
...
//   So in order to identify a set multiple lists in same window may each need
//   a focus scope. If you imagine an hypothetical
...
TableGetHoveredRow (); // Retrieve *PREVIOUS FRAME* hovered row. This
                       // difference with TableGetHoveredColumn() is the reason
...
// FUNCTIONS ARE A MESS. THEIR SIGNATURE AND BEHAVIOR WILL CHANGE, THEY NEED TO
// BE REFACTORED INTO SOMETHING DECENT. NB: All position are in absolute pixels
