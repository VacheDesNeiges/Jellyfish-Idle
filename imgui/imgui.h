...
#pragma clang diagnostic                                                       \
    ignored "-Wfloat-equal" // warning: comparing floating point with == or !=
                            // is unsafe
...
IMGUI_API ImGuiStyle &GetStyle(); // access the Style structure (colors, sizes).
                                  // Always use PushStyleColor(), PushStyleVar()
...
IMGUI_API void EndFrame(); // ends the Dear ImGui frame. automatically called by
                           ...
                           // already! If you don't need to render, better to
                           // not create any windows and not call NewFrame() at
                           // all!
...
IMGUI_API void ShowUserGuide(); // add basic help/info block (not a window): how
                                // to manipulate ImGui as an end-user
...
IMGUI_API bool TableNextColumn(); // append into the next column (or first
                                  // column of next row if currently in last
                                  // column). Return true when column is
                                  // visible.
...
IMGUI_API void TableHeadersRow(); // submit a row with headers cells based on
                                  // data provided to TableSetupColumn() +
                                  // submit context menu
IMGUI_API void TableAngledHeadersRow(); // submit a row with angled headers for
...
IMGUI_API bool IsWindowDocked(); // is current window docked into another
                                 // window?
...
// Overlapping mode
IMGUI_API void SetNextItemAllowOverlap(); // allow next item to be overlapped by
                                          // a subsequent item. Useful with
                                          // invisible buttons, selectable,
                                          // treenode covering an area where
                                          // subsequent items may need to be
                                          // added. Note that both Selectable()
                                          // and TreeNode() have dedicated flags
                                          // doing this.
...
IMGUI_API bool IsAnyMouseDown(); // [WILL OBSOLETE] is any mouse button held?
                                 // This was designed for backends, but prefer
                                 // having backend maintain a mask of held mouse
                                 // buttons, because upcoming input queue system
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackResize =
        1 << 21, // Callback on buffer capacity changes request (beyond
...
struct ImGuiIO
{
    ...
    IMGUI_API void ClearInputKeys();   // Clear current keyboard/mouse/gamepad
                                     ...
                                     // Equivalent to releasing all
                                     // keys/buttons.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    IMGUI_API void ClearInputCharacters(); // [Obsoleted in 1.89.8] Clear the
...
struct ImFontAtlas
{
    ...
    IMGUI_API const ImWchar *GetGlyphRangesDefault();  // Basic Latin, Extended
                                                       // Latin
    IMGUI_API const ImWchar *GetGlyphRangesGreek();    // Default + Greek and
                                                       // Coptic
...
IMGUI_API void SetItemAllowOverlap(); // Use SetNextItemAllowOverlap() before
                                      // item.
