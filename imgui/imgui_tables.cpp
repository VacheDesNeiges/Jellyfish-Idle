...
//    TableUpdateColumnsWeightFromWidth()- recompute columns weights (of
//    stretch columns) from their respective width
...
//    | TableBeginCell()                        - enter into current
//    column/cell
// - [...]                                      user emit contents
//-----------------------------------------------------------------------------
...
//   Y with ScrollX/ScrollY disabled: we output table directly in current
//   window
...
//   scroll.
//   - outer_size.y  = 0.0f  ->  No minimum height (but will auto extend,
//   unless _NoHostExtendY is set)
//   - outer_size.y  > 0.0f  ->  Set Minimum height (but will auto extend,
//   unless _NoHostExtendY is set) Y with ScrollX/ScrollY enabled: using a
//   child window for scrolling
//   - outer_size.y  < 0.0f  ->  Bottom-align. Not meaningful if parent window
//   can vertically scroll.
//   - outer_size.y  = 0.0f  ->  Bottom-align, consistent with BeginChild().
//   Not recommended unless table is last item in parent window.
...
// would be consistent but perhaps less useful and more confusing (as
// vertically clipped items are not useful and not easily noticeable).
//-----------------------------------------------------------------------------
// About 'inner_width':
...
//   outer_size.x) <-- weird
//   - inner_width  = 0.0f  ->  fit in outer_width: Fixed size columns will
//   take space they need (if avail, otherwise shrink down), Stretch columns
//   becomes Fixed columns.
//   - inner_width  > 0.0f  ->  override scrolling width, generally to be
//   larger than outer_size.x. Fixed column take space they need (if avail,
...
//   --> weight is custom
// Widths are specified _without_ CellPadding. If you specify a width of
// 100.0f, the column will be cover (100.0f + Padding * 2.0f) and you can fit a
// 100.0f wide item in it without clipping and with padding honored.
//-----------------------------------------------------------------------------
...
//   Fixed columns, followed by one or two TRAILING Stretch columns.
...
//   contents.
//   - when using ImGuiTableFlags_SizingStretchSame with mixed columns, only
//   the Stretch columns will match their weights/widths.
//-----------------------------------------------------------------------------
...
//   auto-resize. So, if you want to use the clipper, make sure to either
//   enable _Resizable, either setup columns width explicitly with _WidthFixed.
//-----------------------------------------------------------------------------
...
// - Case A: column is not hidden by user, and at least partially in sight
// (most common case).
// - Case B: column is clipped / out of sight (because of scrolling or parent
// ClipRect): TableNextColumn() return false as a hint but we still allow
// layout output.
// - Case C: column is hidden explicitly by the user (e.g. via the context
// menu, or _DefaultHide column flag, etc.).
//
//                        [A]         [B]          [C]
...
//           when ClipRect is zero, ItemAdd() will return false and most
//           widgets will early out mid-way.
//  ImDrawList output:    normal      dummy        dummy       -> [internal]
//  when using the dummy channel, ImDrawList submissions (if any) will be
...
bool
ImGui::BeginTable (const char *str_id, int columns_count,
...
bool
ImGui::BeginTableEx (const char *name, ImGuiID id, int columns_count,
                     ImGuiTableFlags flags, const ImVec2 &outer_size,
                     float inner_width)
{
  ImGuiContext &g = *GImGui;
  ...
  const bool outer_window_is_measuring_size
      = (outer_window->AutoFitFramesX > 0)
        || (outer_window->AutoFitFramesY
            > 0); // Doesn't apply to auto-fitting windows!
  ...
  if (use_child_window)
    {
      ...
      // Ensure specified width (when not specified, Stretched columns will act
      // as if the width == OuterWidth and never lead to any scrolling). We
      // don't handle inner_width < 0.0f, we could potentially use it to
  ...
  // but will reduce cases were "out of table" output will be misleading to the
  // user. Because we cannot safely assert in EndTable() when no rows have been
...
// extending
static void
TableSetupColumnFlags (ImGuiTable *table, ImGuiTableColumn *column,
...
// BeginTable() and this is our largest function. Runs on the first call to
// TableNextRow(), to give a chance for TableSetupColumn() and other
// TableSetupXXXXX() functions to be called first.
...
// widgets relying on WorkRect.Max.x... Maybe provide a default distribution
// for _WidthAuto columns?
void
ImGui::TableUpdateLayout (ImGuiTable *table)
{
  ...
  for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
      ...
      // resubmission anyway. We take a slight shortcut but in theory we could
      // be calling TableSetupColumn() here with dummy values, it should yield
      // the same effect.
  ...
  for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
      ...
      // padding) makes things more consistent when resizing down, tho slightly
      ...
      // Note that scrolling tables (where inner_window != outer_window) handle
      // Y clipped earlier in BeginTable() so IsVisibleY really only applies to
      ...
      // last known content width for clipped column. Test
  ...
  // we keep a column visible. Else if give no chance to a clipper-savy user to
  ...
  table->BorderX1
      = table->InnerClipRect.Min.x
        + ((table->Flags & ImGuiTableFlags_BordersOuterV) ? 1.0f : 0.0f);
...
ImGui::TableUpdateBorders (ImGuiTable *table)
{
  ...
  // so we rely on temporal coherency and use the final height from last frame.
  // Because this is only affecting _interaction_ with columns, it is not
...
// Change user accessible enabled/disabled state of a column (often perceived
// as "showing/hiding" from users point of view) Note that end-user can use the
...
// - Request will be applied during next layout, which happens on the first
// call to TableNextRow() after BeginTable().
...
// We allow querying for an extra column in order to poll the IsHovered state
// of the right-most section
...
// - Important: we generally don't know our row height until the end of the
// row, so Max.y will be incorrect in many situations.
...
//   outer border.
//   FIXME: But the rendering code in TableEndRow() nullifies that with
//   clamping required for scrolling.
ImRect
...
ImGuiID
ImGui::TableGetColumnResizeID (ImGuiTable *table, int column_n,
                               int instance_no)
{
  IM_ASSERT (column_n >= 0 && column_n < table->ColumnsCount);
      ...
      if (draw_cell_bg_color)
        {
          ...
          for (ImGuiTableCellData *cell_data = &table->RowCellData[0];
            ...
            {
              // As we render the BG here we need to clip things (for layout we
              // would not)
...
ImGui::TableSetColumnWidth (int column_n, float width)
{
  ...
  // and F2 would need to stretch down, which would be lossy or ambiguous.
  // Seems hard to fix.
...
// - We only use the dummy draw channel so we can push a null clipping
// rectangle into it without affecting other
//   channels, while simplifying per-row/per-cell overhead. It will be empty
//   and discarded when merged.
...
ImGui::TableSetupDrawChannels (ImGuiTable *table)
{
  ...
  table->DummyDrawChannel = (ImGuiTableDrawChannelIdx)((channels_for_dummy > 0)
                                                           ? channels_total - 1
                                                           : -1);
  ...
  // host, to facilitate merge draw commands by default. All our cell highlight
  // are manually clipped with BgClipRect. When unfreezing it will be made
  // smaller to fit scrolling rect. (This technically isn't part of setting up
  // draw channels, but is reasonably related to be done here)
...
//
// Columns where the contents didn't stray off their local clip rectangle can
// be merged. To achieve this we merge their clip rect and make them contiguous
// in the channel list, so they can be merged by the call to
// DrawSplitter.Merge() following to the call to this function. We reorder draw
// commands by arranging them into a maximum of 4 distinct groups:
//
...
//
// Each column itself can use 1 channel (row freeze disabled) or 2 channels
// (row freeze enabled). When the contents of a column didn't stray off its
// limit, we move its channels into the corresponding group based on its
// position (within frozen rows/columns groups or not). At the end of the
// operation our 1-4 groups will each have a ImDrawCmd using the same ClipRect.
// This function assume that each column are pointing to a distinct draw
// channel, otherwise merge_group->ChannelsCount will not match set bit count
// of merge_group->ChannelsMask.
//
...
//   SetCursorScreenPos().
// - The channel uses more than one draw command itself. We drop all our
// attempt at merging stuff here..
//   we could do better but it's going to be rare and probably not worth the
//   hassle.
// Columns for which the draw channel(s) haven't been merged with other will
// use their own ImDrawCmd.
//
  ...
  for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
      ...
      for (int merge_group_sub_n = 0;
           merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
          ...
          // Find out the width of this merge group and check if it will fit in
          // our column (note that we assume that rendering didn't stray on the
          // left direction. we should need a CursorMinPos to detect it)
           ...
           merge_group_n++)
        {
          if (int merge_channels_count
              = merge_groups[merge_group_n].ChannelsCount)
            {
              ...
              // calls can be merged even if outer-most columns have some outer
              // padding offsetting them from their parent ClipRect. The
              ...
              // FIXME-TABLE FIXME-WORKRECT: We are wasting a merge opportunity
              // on tables without scrolling if column doesn't fit within host
              // clip rect, solely because of the half-padding difference
              // between window->WorkRect and window->InnerClipRect.
  ...
  if (table->Flags & ImGuiTableFlags_BordersOuter)
    {
      ...
      // without adding an extra draw call (Without the offset, in outer_window
      // it would be rendered behind cells, because child windows are above
      // their parent. In inner_window, it won't reach out over scrollbars.
      // Another weird solution would be to display part of it in inner window,
      // and the part that's over scrollbars in the outer window..) Either
      // solution currently won't allow us to use a larger border size: the
      // border would clipped.
...
// Return NULL if no sort specs (most often when ImGuiTableFlags_Sortable is
// not set) When 'sort_specs->SpecsDirty == true' you should sort your data. It
// will be true when sorting specs have changed since last call, or the first
// time. Make sure to set 'SpecsDirty = false' after sorting, else you may
// wastefully sort your data every frame! Lifetime: don't hold on this pointer
...
void
ImGui::TableFixColumnSortDirection (ImGuiTable *table,
                                    ImGuiTableColumn *column)
{
  if (column->SortOrder == -1
...
ImGui::TableGetHeaderRowHeight ()
{
  ...
  // than others. If we didn't do that, uneven header height would highlight
  // but smaller one before the tallest wouldn't catch input for all height. In
  // your custom header row you may omit this all together and just call
...
static void
TableSettingsInit (ImGuiTableSettings *settings, ImGuiID id, int columns_count,
  ...
  for (int n = 0; n < table->ColumnsCount; n++, column++, column_settings++)
    {
      ...
      // restore our state. Note that fixed width where initial width was
      // derived from auto-fit will always be saved as InitStretchWeightOrWidth
      // will be 0.0f.
...
static void *
TableSettingsHandler_ReadOpen (ImGuiContext *, ImGuiSettingsHandler *,
                               const char *name)
{
  ImGuiID id = 0;
  int columns_count = 0;
...
static void
TableSettingsHandler_ReadLine (ImGuiContext *, ImGuiSettingsHandler *,
...
// overwrite of clipping rectangle _without_ altering the command-buffer and
// let the subsequent single call to SetCurrentChannel() does it things once.
void
...
static float
GetColumnWidthEx (ImGuiOldColumns *columns, int column_index,
...
void
ImGui::BeginColumns (const char *str_id, int columns_count,
