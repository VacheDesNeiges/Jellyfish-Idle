...
// handling windows, inputs, graphics context creation, etc.) (Prefer
// SDL 2.0.5+ for full feature support.)
...
//  ImGui::IsKeyPressed(ImGuiKey_Space). [Legacy SDL_SCANCODE_* values will
...
//  ImGuiPlatformIO interface. 2024-02-14: Inputs: Handle gamepad
//  disconnection. Added ImGui_ImplSDL2_SetGamepadMode(). 2023-10-05: Inputs:
//  Added support for extra ImGuiKey values: F13 to F24 function keys, app
//  back/forward keys. 2023-04-06: Inputs: Avoid calling
...
//  preciseX/preciseY mouse wheel data for smooth scrolling + Scaling X value
//  on Emscripten (bug?). (#4019, #6096) 2023-02-02: Removed SDL_MOUSEWHEEL
//  value clamping, as values seem correct in latest Emscripten. (#4019)
//  2023-02-01: Flipping SDL_MOUSEWHEEL 'wheel.x' value to match other backends
//  and offer consistent horizontal scrolling direction. (#4019, #6096, #1463)
//  2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11.
//  2022-09-26: Inputs: Disable SDL 2.0.22 new "auto capture"
//  (SDL_HINT_MOUSE_AUTO_CAPTURE) which prevents drag and drop across windows
//  for multi-viewport support + don't capture when drag and dropping. (#5710)
//  2022-09-26: Inputs: Renamed ImGuiKey_ModXXX introduced in 1.87 to
//  ImGuiMod_XXX (old names still supported). 2022-03-22: Inputs: Fix mouse
//  position issues when dragging outside of boundaries. SDL_CaptureMouse()
//  erroneously still gives out LEAVE events when hovering OS decorations.
//  2022-03-22: Inputs: Added support for extra mouse buttons
//  (SDL_BUTTON_X1/SDL_BUTTON_X2). 2022-02-04: Added SDL_Renderer* parameter to
...
//  to fix input queue with very low framerates. 2022-01-12: Update mouse
//  inputs using SDL_MOUSEMOTION/SDL_WINDOWEVENT_LEAVE + fallback to provide it
//  when focused but not hovered/captured. More standard and will allow us to
//  pass it to future input queue API. 2022-01-12: Maintain our own copy of
//  MouseButtonsDown mask instead of using ImGui::IsAnyMouseDown() which will
//  be obsoleted. 2022-01-10: Inputs: calling new io.AddKeyEvent(),
...
//  SDL_WINDOWEVENT_FOCUS_GAINED/SDL_WINDOWEVENT_FOCUS_LOST. 2021-07-29:
//  Inputs: MousePos is correctly reported when the host platform window is
//  hovered but not focused (using SDL_GetMouseFocus() +
//  SDL_HINT_MOUSE_FOCUS_CLICKTHROUGH, requires SDL 2.0.5+) 2021-06:29:
//  *BREAKING CHANGE* Removed 'SDL_Window* window' parameter to
//  ImGui_ImplSDL2_NewFrame() which was unnecessary. 2021-06-29: Reorganized
...
//  Rework global mouse pos availability check listing supported platforms
//  explicitly, effectively fixing mouse access on Raspberry Pi. (#2837, #3950)
//  2020-05-25: Misc: Report a zero display-size when window is minimized, to
//  be consistent with other backends. 2020-02-20: Inputs: Fixed mapping for
//  ImGuiKey_KeyPadEnter (using SDL_SCANCODE_KP_ENTER instead of
//  SDL_SCANCODE_RETURN2). 2019-12-17: Inputs: On Wayland, use
...
//  Inputs: Workaround for Emscripten which doesn't seem to handle focus
//  related calls. 2018-06-29: Inputs: Added support for the
...
//  SDL_GLContext parameter. 2018-05-09: Misc: Fixed clipboard paste memory
//  leak (we didn't call SDL_FreeMemory on the data returned by
//  SDL_GetClipboardText). 2018-03-20: Misc: Setup io.BackendFlags
//  ImGuiBackendFlags_HasMouseCursors flag + honor
//  ImGuiConfigFlags_NoMouseCursorChange flag. 2018-02-16: Inputs: Added
//  support for mouse cursors, honoring ImGui::GetMouseCursor() value.
...
//  2018-02-05: Misc: Using SDL_GetPerformanceCounter() instead of
//  SDL_GetTicks() to be able to handle very high framerate (1000+ FPS).
//  2018-02-05: Inputs: Keyboard mapping is using scancodes everywhere instead
//  of a confusing mixture of keycodes and scancodes. 2018-01-20: Inputs: Added
//  Horizontal Mouse Wheel support. 2018-01-19: Inputs: When available
//  (SDL 2.0.4+) using SDL_CaptureMouse() to retrieve coordinates outside of
//  client area when dragging. Otherwise (SDL 2.0.3 and before) testing for
//  SDL_WINDOW_INPUT_FOCUS instead of SDL_WINDOW_MOUSE_FOCUS. 2018-01-18:
//  Inputs: Added mapping for ImGuiKey_Insert. 2017-08-25: Inputs: MousePos set
//  to -FLT_MAX,-FLT_MAX when mouse is unavailable/missing (instead of -1,-1).
//  2016-10-15: Misc: Added a void* user_data parameter to Clipboard function
//  handlers.
...
// SDL
// (the multi-viewports feature requires SDL features supported from
// SDL 2.0.4+. SDL 2.0.5+ is highly recommended)
#include <SDL2/SDL.h>
...
// branch with multi-viewports (== single Dear ImGui context + multiple
// windows) instead of multiple Dear ImGui contexts.
// FIXME: multi-context support is not well tested and probably dysfunctional
// in this backend.
...
static void
ImGui_ImplSDL2_SetPlatformImeData (ImGuiViewport *viewport,
    ...
    case SDL_DISPLAYEVENT:
      {
        ...
        // so change of DPI/Scaling are not reflected in this event. (SDL3 has
        // it)
    ...
    case SDL_WINDOWEVENT:
      {
        ...
        // LEAVE/ENTER event on every mouse move, but the final ENTER tends to
        // be right.
        ...
        //   operation by clear mouse position. This is why we delay process
        //   the SDL_WINDOWEVENT_LEAVE events by one frame. See issue #5012 for
        //   details.
                     ...
                     void *sdl_gl_context)
{
  ...
  for (int n = 0; n < IM_ARRAYSIZE (global_mouse_whitelist); n++)
    if (strncmp (sdl_backend, global_mouse_whitelist[n],
                 strlen (global_mouse_whitelist[n]))
        == 0)
  ...
  if (SDL_GetWindowWMInfo (window, &info))
    ...
    // otherwise SDL doesn't emit the event. Without this, when clicking to
    // gain focus, our widgets wouldn't activate even though they showed as
    // hovered. (This is unfortunately a global SDL setting, so enabling it
    // might have a side-effect on your application. It is unlikely to make a
    // difference, but if your app absolutely needs to ignore the initial
    // on-focus click: you can ignore SDL_MOUSEBUTTONDOWN events coming right
    // after a SDL_WINDOWEVENT_FOCUS_GAINED)
...
ImGui_ImplSDL2_UpdateMouseData ()
{
  ...
  //       in order to allow us to find the viewport under), then Dear ImGui is
...
static void
ImGui_ImplSDL2_UpdateGamepadAnalog (ImGui_ImplSDL2_Data *bd, ImGuiIO &io,
                                    ImGuiKey key,
  ...
  for (int n = 0; n < display_count; n++)
    {
      ...
      // the application DPI awareness settings, which generally needs to be
...
// and handle multiple viewports simultaneously. If you are new to dear imgui
// or creating a new binding for dear imgui, it is recommended that you
...
static void
ImGui_ImplSDL2_CreateWindow (ImGuiViewport *viewport)
{
  ImGui_ImplSDL2_Data *bd = ImGui_ImplSDL2_GetBackendData ();
...
static void
ImGui_ImplSDL2_DestroyWindow (ImGuiViewport *viewport)
{
...
static void
ImGui_ImplSDL2_ShowWindow (ImGuiViewport *viewport)
{
...
static ImVec2
ImGui_ImplSDL2_GetWindowPos (ImGuiViewport *viewport)
{
  ImGui_ImplSDL2_ViewportData *vd
      = (ImGui_ImplSDL2_ViewportData *)viewport->PlatformUserData;
  int x = 0, y = 0;
...
static void
ImGui_ImplSDL2_SetWindowPos (ImGuiViewport *viewport, ImVec2 pos)
{
  ImGui_ImplSDL2_ViewportData *vd
      = (ImGui_ImplSDL2_ViewportData *)viewport->PlatformUserData;
...
static ImVec2
ImGui_ImplSDL2_GetWindowSize (ImGuiViewport *viewport)
{
  ImGui_ImplSDL2_ViewportData *vd
      = (ImGui_ImplSDL2_ViewportData *)viewport->PlatformUserData;
  int w = 0, h = 0;
...
static void
ImGui_ImplSDL2_SetWindowSize (ImGuiViewport *viewport, ImVec2 size)
{
  ImGui_ImplSDL2_ViewportData *vd
      = (ImGui_ImplSDL2_ViewportData *)viewport->PlatformUserData;
...
static void
ImGui_ImplSDL2_SetWindowTitle (ImGuiViewport *viewport, const char *title)
{
  ImGui_ImplSDL2_ViewportData *vd
      = (ImGui_ImplSDL2_ViewportData *)viewport->PlatformUserData;
...
static void
ImGui_ImplSDL2_SetWindowAlpha (ImGuiViewport *viewport, float alpha)
{
  ImGui_ImplSDL2_ViewportData *vd
      = (ImGui_ImplSDL2_ViewportData *)viewport->PlatformUserData;
...
static void
ImGui_ImplSDL2_SetWindowFocus (ImGuiViewport *viewport)
{
  ImGui_ImplSDL2_ViewportData *vd
      = (ImGui_ImplSDL2_ViewportData *)viewport->PlatformUserData;
...
static bool
ImGui_ImplSDL2_GetWindowFocus (ImGuiViewport *viewport)
{
  ImGui_ImplSDL2_ViewportData *vd
      = (ImGui_ImplSDL2_ViewportData *)viewport->PlatformUserData;
...
static bool
ImGui_ImplSDL2_GetWindowMinimized (ImGuiViewport *viewport)
{
  ImGui_ImplSDL2_ViewportData *vd
      = (ImGui_ImplSDL2_ViewportData *)viewport->PlatformUserData;
...
static void
ImGui_ImplSDL2_RenderWindow (ImGuiViewport *viewport, void *)
{
  ImGui_ImplSDL2_ViewportData *vd
      = (ImGui_ImplSDL2_ViewportData *)viewport->PlatformUserData;
  if (vd->GLContext)
...
static void
ImGui_ImplSDL2_SwapBuffers (ImGuiViewport *viewport, void *)
{
  ImGui_ImplSDL2_ViewportData *vd
      = (ImGui_ImplSDL2_ViewportData *)viewport->PlatformUserData;
  if (vd->GLContext)
...
static int
ImGui_ImplSDL2_CreateVkSurface (ImGuiViewport *viewport, ImU64 vk_instance,
...
static void
ImGui_ImplSDL2_InitPlatformInterface (SDL_Window *window, void *sdl_gl_context)
{
